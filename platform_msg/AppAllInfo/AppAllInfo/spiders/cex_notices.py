...
import codecs  
class cex_notices_spider(scrapy.Spider):
    name = "cex_notices_spider"
    allowed_domains = ["cex.com"]
    urls = [
        #"http://www.wandoujia.com/tag/视频",
        #"http://www.dmoz.org/Computers/Programming/Languages/Python/Resources/"

        "https://cex.com/Art/index/id/1.html"
...
#rules = [Rule(LinkExtractor(allow=['/apps/.+']), 'parse')]
    def parse(self, response):
        ...
        linklist = [link for link in  page.xpath("//a/@href") if link.extract().startswith("/Art/details/id/") and link.extract().strip() != "https://cex.com/Art/index/id/1.html" ]
        href=linklist[0].extract()

            #if href.startswith("/hc/zh-cn/articles"):
        print "++++++++++++++++++++++",href
        yield Request("https://cex.com" +href, callback=self.parse_new_page)
    ...
    def parse_new_page(self, response):
        ...
        title = sel.css("body > div.container > div > div.col-xs-10.padding-right-clear > div > div.panel-body > div > h1").extract()
        content = sel.css('''body > div.container > div > div.col-xs-10.padding-right-clear > div > div.panel-body > div > div.txt''').extract()
